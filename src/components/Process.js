...
const Process = () => {
    ...
    return (
        <section id="proses-kerja" className="py-32 md:py-40 bg-white text-navy overflow-hidden relative border-t border-navy/5 scroll-mt-20">
            {/* Subtle background pattern - Blue Aura glow */}
            <div className="absolute inset-0 opacity-[0.2] pointer-events-none">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-[radial-gradient(circle_at_center,rgba(30,58,138,0.05)_0%,transparent_70%)]"></div>
            </div>
            ...
            <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
                <div className="mb-24 md:mb-32 flex flex-col lg:flex-row lg:items-end justify-between gap-12">
                    <div className="max-w-3xl">
                        <div className="flex items-center gap-4 mb-8">
                            ...
                            <span className="label-overline text-gold">Alur Kerja</span>
                        </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6" role="list" aria-label="Proses perizinan 4 tahap">
                    {steps.map((step, i) => (
                        <div key={i} className="relative p-10 lg:p-12 bg-[#FBF9F7] border border-navy/5 hover:border-gold/20 hover:bg-white hover:shadow-[0_20px_40px_-15px_rgba(30,58,138,0.05)] transition-all duration-700 group overflow-hidden rounded-sm" role="listitem">
                            {/* Hover Background Accent */}
                            <div className="absolute top-0 right-0 w-40 h-40 bg-navy/[0.02] blur-[80px] translate-x-20 -translate-y-20 group-hover:translate-x-10 group-hover:-translate-y-10 transition-transform duration-1000"></div>

                            <span className="text-8xl font-extrabold text-navy/[0.02] absolute -top-4 -right-4 group-hover:text-gold/[0.04] transition-colors duration-700 tracking-tighter">{step.num}</span>

                            <div className="relative z-10 h-full flex flex-col justify-between">
                                <div className="mb-12">
                                    <div className="w-10 h-10 rounded-full border border-navy/5 flex items-center justify-center text-gold mb-10 bg-white shadow-sm group-hover:bg-gold group-hover:text-white transition-all duration-500 text-xs font-bold">
                                        {step.num}
                                    </div>
                                ...
                                <div className="w-full h-[1px] bg-navy/5 group-hover:bg-gold/20 transition-colors duration-700"></div>
                            </div>
                        </div>
                    ))}
                </div>
